...
with open("notes.txt", "r", encoding="utf-8") as dosya:

    for satir in dosya:

        try: 
            not_degeri = int(satir.strip()) # satir eşit değildir int ise burada hata olacak, strip() --> boşluk varsa çıkartması için
            notlar.append(not_degeri)
        except ValueError: # Hata olduğunda exception'a düşecek.
